...
export const CategoryPage: React.FC<ICategoryProps> = (props) => {
  ...
  const { openModal, currentCategory, setCurrentCategory } = useStore();
  ...
  return (
    <PageWrapper>
      <AppBar
        onBackClicked={() => (searchBoxFocused ? setSearchBoxFocused(false) : navigate("/"))}
        onDoneClicked={searchBoxFocused ? () => setSearchBoxFocused(false) : undefined}
        title={category?.name}
      />
